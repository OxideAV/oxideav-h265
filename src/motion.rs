...
impl MotionField {
    ...
    /// Copy out the cells covering the luma rectangle `[(x0, y0),
    /// (x0 + w, y0 + h))` (clipped to the field), row-major — the
    /// encoder's coding-quadtree search restores them with
    /// [`Self::restore_rect`] when it rolls a trial decision back.
    #[must_use]
