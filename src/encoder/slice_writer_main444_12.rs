//! 12-bit IDR I-slice emitter for the Main 4:4:4 12 profile (round 32).
//!
...
//! the 12-bit pipeline of [`super::slice_writer_main12`] (`u16` sample
//! containers, `bit_depth = 12` threaded through every
//! `intra_pred::predict` / `transform::*` call, and the §8.6.1
//! `Qp'Y = SliceQpY + QpBdOffsetY = 26 + 24 = 50` quantiser used on
//! both luma and chroma).
//!
//! Like the round-25 / 26 / 30 / 31 parallel writers, this module
//! mirrors rather than templates the existing paths so the round-1..29
//! (8-bit 4:2:0), round-25 (Main 10), round-26 (Main 12), round-30
//! (Main 4:4:4 8-bit), and round-31 (Main 4:4:4 10) emissions stay
//! byte-for-byte unchanged.
//!
//! Source pixel layout: `Yuv444P12Le` — three planes of 16-bit
//! little-endian samples, each plane the same width × height as the
//! luma. The 4:4:4 helper [`build_idr_slice_nal_main444_12`] reads each
//! sample via [`read_p12_sample`] (LE 16-bit unpack, low 12 bits valid)
//! just like the Main 12 path; the only difference is that the Cb / Cr
//! planes have the same `width × height` (and the same byte stride) as
//! luma.
...
/// step and the cross-decode collapses (the same failure mode the
/// round-25 / 26 / 31 high-bit-depth work hit before each QpBdOffset
/// fix).
...
/// Build Annex B bytes for the IDR I-slice NAL using the 12-bit 4:4:4 pipeline.
pub fn build_idr_slice_nal_main444_12(cfg: &EncoderConfig, frame: &VideoFrame) -> Vec<u8> {
    let rbsp = build_idr_slice_rbsp_main444_12(cfg, frame);
    build_annex_b_nal(NalHeader::for_type(NalUnitType::IdrNLp), &rbsp)
}

fn build_idr_slice_rbsp_main444_12(cfg: &EncoderConfig, frame: &VideoFrame) -> Vec<u8> {
    ...
    let mut enc = EncoderState444_12::new(cfg);
    {
...
impl EncoderState444_12 {
    ...
    fn encode_ctu(&mut self, cw: &mut CabacWriter<'_>, src: &VideoFrame, x0: u32, y0: u32) {
        ...
        // MPM list + signalling — same as the 4:2:0 / 4:4:4-8 / 4:4:4-10 paths.
...
/// Read a single 12-bit sample from a `Yuv444P12Le` plane. The plane
/// data is laid out as little-endian 16-bit words; `stride` is the
/// byte stride (not sample stride). Only the low 12 bits are valid.
fn read_p12_sample(plane: &[u8], stride: usize, x: usize, y: usize) -> u16 {
    let off = y * stride + x * 2;
    (plane[off] as u16) | ((plane[off + 1] as u16) << 8)
}

/// Sum-absolute-difference between an n×n prediction (12-bit u16) and
/// a region of a 12-bit packed source plane.
...
fn chroma_qp_for_slice() -> i32 {
    ...
    // zero offsets, Qp'Cb = 26 + 24 = 50 at 12-bit. Same for Cr.
    SLICE_QP_PRIME_Y
}
...
mod tests {
    ...
    /// Pack a u16 plane into the `Yuv444P12Le` byte layout (little-endian).
    ...
    fn make_gradient_yuv444p12_frame(w: usize, h: usize) -> VideoFrame {
        ...
        let cb = vec![2048u16; w * h];
        let cr = vec![2048u16; w * h];
        VideoFrame {
            pts: Some(0),
            planes: vec![
                VideoPlane {
    ...
    fn local_recon_close_to_input_qp26() {
        ...
        // the 4:4:4 path. PSNR is computed in the 12-bit range
        // (max sample = 4095).
        let cfg = EncoderConfig::new_main444_12(16, 16);
        let state = EncoderState444_12::new(&cfg);
        let frame = make_gradient_yuv444p12_frame(16, 16);
        ...
        eprintln!("main444_12 local recon psnr={psnr:.2} dB");
        assert!(
            psnr > 30.0,
            "Main 4:4:4 12 local PSNR too low: {psnr:.2} dB"
        );
    }

    #[test]
    fn idr_emits_with_main444_12_layout() {
        let cfg = EncoderConfig::new_main444_12(16, 16);
        let frame = make_gradient_yuv444p12_frame(16, 16);
        let nal = build_idr_slice_nal_main444_12(&cfg, &frame);
