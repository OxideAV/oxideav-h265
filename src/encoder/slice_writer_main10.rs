//! 10-bit IDR I-slice emitter (Main 10 profile).
//!
//! Mirrors the structure of [`super::slice_writer`] but operates on 10-bit
//! `u16` samples throughout. The CTU layout is identical to the 8-bit
//! path (16×16 luma, 8×8 chroma TBs, intra DC/planar/cardinals/diagonals
//! mode set, fixed QP 26, single 2Nx2N PU per CB) — only the sample
//! container width and the `bit_depth` argument threaded through
//! `intra_pred::predict` / `transform::*` differ.
//!
//! Keeping a parallel implementation rather than templating the existing
//! 8-bit `EncoderState` over bit depth lets the Main (8-bit) emission
//! stay byte-for-byte unchanged across rounds 1..24 — every existing
//! cross-decode and round-trip test still hits exactly the same bitstream
//! it did before round 25.
...
/// dequantiser sees is `SLICE_QP_Y + QP_BD_OFFSET = 38` for both luma
...
/// the reconstruction overshoots the source by a factor of `2^12` / step
/// and the cross-decode collapses to ~10 dB.
...
/// Build Annex B bytes for the IDR I-slice NAL using the 10-bit pipeline.
pub fn build_idr_slice_nal_main10(cfg: &EncoderConfig, frame: &VideoFrame) -> Vec<u8> {
    let rbsp = build_idr_slice_rbsp_main10(cfg, frame);
    build_annex_b_nal(NalHeader::for_type(NalUnitType::IdrNLp), &rbsp)
}

/// Build the 10-bit IDR I-slice RBSP and return the encoder's local
/// reconstruction (10-bit `u16` planes) for use as a reference in a
/// later P/B emission. P/B at 10-bit is **not yet implemented**, so the
/// returned reconstruction is currently only consumed by self-tests.
pub fn build_idr_slice_with_reconstruction_main10(
    cfg: &EncoderConfig,
    frame: &VideoFrame,
) -> (Vec<u8>, Reconstruction10) {
    let (rbsp, recon) = build_idr_slice_rbsp_with_reconstruction_main10(cfg, frame);
    let nal = build_annex_b_nal(NalHeader::for_type(NalUnitType::IdrNLp), &rbsp);
    (nal, recon)
}

/// 10-bit reconstruction handed back from
/// [`build_idr_slice_with_reconstruction_main10`]. Plane data is packed
/// in row-major layout, one sample per `u16`.
#[derive(Clone)]
pub struct Reconstruction10 {
    pub width: u32,
    pub height: u32,
    pub y: Vec<u16>,
    pub cb: Vec<u16>,
...
fn build_idr_slice_rbsp_with_reconstruction_main10(
    cfg: &EncoderConfig,
    frame: &VideoFrame,
) -> (Vec<u8>, Reconstruction10) {
    ...
    let mut enc = EncoderState10::new(cfg);
    {
...
fn build_idr_slice_rbsp_main10(cfg: &EncoderConfig, frame: &VideoFrame) -> Vec<u8> {
    let (rbsp, _recon) = build_idr_slice_rbsp_with_reconstruction_main10(cfg, frame);
    rbsp
}

struct EncoderState10 {
    cfg: EncoderConfig,
    rec_y: Vec<u16>,
    rec_cb: Vec<u16>,
    rec_cr: Vec<u16>,
...
/// Read a single 10-bit sample from a `Yuv420P10Le` plane. The plane data
...
/// Sum-absolute-difference between an n×n prediction (10-bit u16) and a
/// region of a 10-bit packed `Yuv420P10Le` source plane.
...
fn chroma_qp_for_slice() -> i32 {
    ...
    // Qp'Cb = 26 + 12 = 38 at 10-bit. Same for Cr.
    SLICE_QP_PRIME_Y
}
...
mod tests {
    ...
    /// Pack a u16 plane into the `Yuv420P10Le` byte layout (little-endian).
    ...
    fn make_gradient_p10_frame(w: usize, h: usize) -> VideoFrame {
        ...
        let cw = w / 2;
        let ch = h / 2;
        let cb = vec![512u16; cw * ch];
        let cr = vec![512u16; cw * ch];
        VideoFrame {
            pts: Some(0),
            planes: vec![
                VideoPlane {
    ...
    fn local_reconstruction_close_to_input_at_qp26() {
        ...
        // PSNR is computed in the 10-bit range (max sample = 1023).
        let cfg = EncoderConfig::new_main10(16, 16);
        let state = EncoderState10::new(&cfg);
        let frame = make_gradient_p10_frame(16, 16);
        ...
        eprintln!("main10 local recon psnr={psnr:.2} dB");
        assert!(psnr > 30.0, "10-bit local PSNR too low: {psnr:.2} dB");
    }

    #[test]
    fn idr_emits_with_main10_sps() {
        // End-to-end: a 16x16 Main 10 frame produces a valid IDR slice
        // body that decodes back through the parameter-set parser.
        let cfg = EncoderConfig::new_main10(16, 16);
        let frame = make_gradient_p10_frame(16, 16);
        let nal = build_idr_slice_nal_main10(&cfg, &frame);
