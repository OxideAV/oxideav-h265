//! Round-31: Main 4:4:4 10 (10-bit, 4:4:4) encoder integration tests.
//!
//! Scope:
//! * The encoder accepts a `Yuv444P10Le` keyframe and emits an Annex B
//!   stream whose SPS declares Main 4:4:4 10 (`profile_idc = 4`,
//!   `chroma_format_idc = 3`, `bit_depth_*_minus8 = 2`) with the
//!   §A.3.4 RExt constraint flags row for Main 4:4:4 10
//!   (`max_8bit = 0`, `max_10bit = 1`, `max_12bit = 1`,
//!   `max_422chroma = 0`, `max_420chroma = 0`).
//! * Our own decoder reads the bitstream back at 10-bit 4:4:4 (chroma
...
//!   (PSNR ≥ 30 dB on the 10-bit / peak = 1023 scale).
...
//! * `mini_gop > 1` is rejected at construction time (4:4:4 + 10-bit is
//!   IDR-only — same restriction the round-30 8-bit 4:4:4 path applies).
...
/// Build a `Yuv444P10Le` frame with a horizontal 0..1023 luma gradient
/// and flat mid-range chroma (512). Plane data follows the LE 16-bit
/// layout used by `oxideav-core`'s `PixelFormat::Yuv444P10Le`. At 4:4:4
/// the chroma planes are full luma resolution so each plane is
/// `w * h * 2` bytes.
fn make_p444p10_gradient_frame(w: u32, h: u32) -> VideoFrame {
    let wu = w as usize;
    let hu = h as usize;
    ...
    for _ in 0..(wu * hu) {
        // 10-bit chroma neutral = 512 = 0x0200 → LE bytes 0x00, 0x02.
        cb.push(0x00);
        cb.push(0x02);
        cr.push(0x00);
        cr.push(0x02);
...
fn p444p10_params(w: u32, h: u32) -> CodecParameters {
    let mut p = CodecParameters::video(CodecId::new("h265"));
    p.width = Some(w);
    p.height = Some(h);
    p.pixel_format = Some(PixelFormat::Yuv444P10Le);
    p.frame_rate = Some(Rational::new(30, 1));
    p
}

fn encode_one_p444p10(frame: &VideoFrame, w: u32, h: u32) -> Vec<u8> {
    let params = p444p10_params(w, h);
...
/// Read a single 10-bit sample from a packed `Yuv*P10Le` plane.
...
#[test]
fn main444_10_emit_carries_main444_10_sps() {
    let frame = make_p444p10_gradient_frame(64, 64);
    let bytes = encode_one_p444p10(&frame, 64, 64);
    let nals: Vec<_> = iter_annex_b(&bytes).collect();
    // VPS + SPS + PPS + IDR — same NAL count as the 4:2:0 / 4:4:4-8 emissions.
    ...
    // Main 4:4:4 10 lives in the Format Range Extensions profile per
    // §A.3.4, so general_profile_idc = 4.
    assert_eq!(sps.profile_tier_level.general_profile_idc, 4);
    // §A.3.4 Table A.2 row for Main 4:4:4 10 — verify the 9 RExt
    ...
    // round-31 emitter writes the exact §A.3.4 signature for Main
    // 4:4:4 10: max_12bit=1, max_10bit=1, max_8bit=0,
    ...
    // intra=0, one_picture_only=0, lower_bit_rate=1.
    {
        ...
        assert_eq!(br.u1().unwrap(), 1, "max_10bit must be 1");
        assert_eq!(br.u1().unwrap(), 0, "max_8bit must be 0 for Main 4:4:4 10");
...
#[test]
fn main444_10_self_roundtrip_psnr() {
    let src = make_p444p10_gradient_frame(64, 64);
    let bytes = encode_one_p444p10(&src, 64, 64);
    ...
    // At 4:4:4 + 10-bit, every plane is 16-bit packed at full luma
    ...
    eprintln!("main444_10 self-roundtrip 64x64: psnr_y={psnr:.2} dB (peak=1023)");
    assert!(psnr > 30.0, "10-bit 4:4:4 luma PSNR too low: {psnr:.2} dB");
}

#[test]
fn main444_10_multi_ctu_roundtrip_psnr() {
    let src = make_p444p10_gradient_frame(128, 128);
    let bytes = encode_one_p444p10(&src, 128, 128);
    let mut dec = HevcDecoder::new(CodecId::new("h265"));
    let pkt = Packet::new(0, TimeBase::new(1, 30), bytes);
    dec.send_packet(&pkt).expect("send packet");
    let frame = match dec.receive_frame() {
    ...
    eprintln!("main444_10 self-roundtrip 128x128: psnr_y={psnr:.2} dB (peak=1023)");
    assert!(psnr > 30.0, "10-bit 4:4:4 luma PSNR too low: {psnr:.2} dB");
}
...
#[test]
fn main444_10_ffmpeg_cross_decode_psnr() {
    if !have_ffmpeg() {
        eprintln!("ffmpeg gate disabled — skipping Main 4:4:4 10 ffmpeg cross-decode");
        return;
    }
    let w = 64u32;
    let h = 64u32;
    let src = make_p444p10_gradient_frame(w, h);
    let bytes = encode_one_p444p10(&src, w, h);
    ...
    let ns = std::time::SystemTime::now()
        ...
        .unwrap_or(0);
    let stem = format!(
        "oxideav_h265_main444_10_{}_{w}x{h}_{ns}",
        std::process::id()
    );
    ...
    if !out.status.success() {
        ...
        panic!(
            "ffmpeg rejected the Main 4:4:4 10 bitstream (exit {:?})",
            out.status
        );
    }
    ...
    eprintln!("ffmpeg Main 4:4:4 10 cross-decode 64x64 gradient: psnr_y={psnr:.2} dB (peak=1023)");
    // Acceptance: ≥ 40 dB Y on a synthetic gradient (round-31 acceptance).
    assert!(
        psnr >= 40.0,
...
#[test]
fn main444_10_rejects_mini_gop_two() {
    // 10-bit + 4:4:4 + mini_gop=2 (B slices) should fail at construction
    // time since both constraints (high-bit-depth and 4:4:4) currently
    // restrict the encoder to keyframe-only.
    let params = p444p10_params(64, 64);
    let res = HevcEncoder::from_params_with_mini_gop(&params, 2);
    assert!(
        res.is_err(),
        "Main 4:4:4 10 should reject mini_gop=2 for now"
    );
}
