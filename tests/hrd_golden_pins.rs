...
//! same black-box reference decoder (ffmpeg 8.1.2 CLI) — and once
//! more when the integer motion search gained its two-start grid +
//! square-search form. The pyramid pin was re-pinned a third time
//! when the flush tail became a short (non-dyadic) mini-GOP instead
//! of a low-delay P chain (its last four frames are now
//! `P, B, B, B` with the reorder schedule carried in
//! `pic_dpb_output_delay`); re-validated the same way.
